...
namespace GiiGa
{
    ...
    {
    public:
        Mesh(IRenderContext& render_context, RenderDevice& device, const std::vector<VertexPNTBT>& vertices,
            const std::vector<Index16>& indices):
            vertexBuffer_(device, CD3DX12_RESOURCE_DESC::Buffer(vertices.size() * sizeof(VertexPNTBT), D3D12_RESOURCE_FLAG_NONE)),
            indexBuffer_(device, CD3DX12_RESOURCE_DESC::Buffer(indices.size() * sizeof(Index16), D3D12_RESOURCE_FLAG_NONE))
        {
            const auto vertices_span = std::span{reinterpret_cast<const uint8_t*>(vertices.data()), vertices.size() * sizeof(VertexPNTBT)};
            ...
            vertexView_ = vertexBuffer_.CreateVetexBufferView(
                D3D12_VERTEX_BUFFER_VIEW{0, static_cast<UINT>(vertices.size() * sizeof(VertexPNTBT)), sizeof(VertexPNTBT)});
